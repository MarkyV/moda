...
public class NameMatch {
    ...
    public Match BestMatch(String Orig) {
        ...
        if (ValidSet == null) {
            return bm; // no valid values, return null
        }
        ...
        if (Orig.replaceAll("[0-9]", "").length() < minLen) {
            return bm; // string is too short to spellcheck
        }

        for (String potentialMatch : ValidSet) {
            currentScore = score(Orig.toUpperCase(), potentialMatch.toUpperCase());
            if (currentScore > bm.score) {
                continue; // no better than other matches
            }
            bm.score = currentScore;
            bm.match = potentialMatch;
        }

        if (bm.score > maxDiff || (double) bm.score / (double) Orig.length() > diffProportion) {
            bm.match = "";
        }
    ...
    public Match BestMatch(String matchByVal, String Orig) {
        Match bm = new Match("", 999);
        if (ValidHash == null) {
            throw new RuntimeException("Group variable not specified in namematch constructor");
        }
    ...
    public Match BestMatch(ArrayList<String> matchByVals, String Orig) {
        ...
        if (matchByVals.size() > 0) {
            for (String matchByVal : matchByVals)
                ValidSet.addAll(ValidHash.get(matchByVal));
        } else {
            ValidSet = FullValidSet;
        }
        bm = BestMatch(Orig);
        return bm;
    }
