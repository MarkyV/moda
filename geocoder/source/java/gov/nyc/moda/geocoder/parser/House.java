...
public class House {
    ...
    public House(String unparsed) {
        ...
        if (!Parser.anydigit(unparsed) && !Parser.anyalpha(unparsed)) {
            return;
        }

        while (unparsed.indexOf("--") >= 0) {
        ...
        if (matcher.find()) {
            ...
            if (unit.length() < unparsed.length()) {
                housenum = unparsed.substring(0, unparsed.length() - unit.length());
            } else {
                housenum = "";
            }
        } else {
        ...
        if (housenum.length() > 1 && housenum.substring(housenum.length() - 1).equals("-")) {
            housenum = housenum.substring(0, housenum.length() - 1);
        }

        //set numeric
        if (housenum.replaceAll("[^0-9]", "").length() > 0) {
            num = Long.parseLong(housenum.replaceAll("[^0-9]", ""));
        }
    }

    public String toString() {
        if (unit.contains("/")) {
            return (housenum + " " + unit).trim();
        } else {
            return (housenum + unit).trim();
        }
    }
    ...
    public static ArrayList<House> Houses(String unparsed) {
        ...
        if (!unparsed.contains(" ")) {
            ...
            if (house.num > 0 || house.unit.length() > 0) {
                houses.add(house);
            }
            ...
            if (house.housenum.contains("-") && house.unit.equals("")) {
                ...
                if (!Parser.notdigit(s[0]) && !Parser.notdigit(s[1]) && Long.parseLong(s[0]) <= Long.parseLong(s[1]) && Long.parseLong(s[1]) - Long.parseLong(s[0]) <= 12) {
                    house = new House(s[1]);
                    if (house.num > 0 || house.unit.length() > 0) {
                        houses.add(house);
                    }
        ...
        //multiple tokens
            ...
            if (housesSplit[1].contains("/")) {
                ...
                for (int i = 0; i < housesSplit.length; i++) {
