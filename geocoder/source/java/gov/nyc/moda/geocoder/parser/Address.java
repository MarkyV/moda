...
public class Address {
    ...
    public Address(Parser p, ArrayList<String> boros, String unparsed_address) {
        ...
        if (s.contains(" ") && Parser.anydigit(s.substring(0, 1))) {
            ...
            if (spl[1].contains("/") && !Parser.notdigit(spl[1].replaceAll("/", "")) && spl[1].substring(0, 1).equals("1")) {
                h = h + " " + spl[1];
            }

            //alpha house number suffix split from house number
            if (spl[1].length() == 1 && Parser.anyalpha(spl[1]) && !Arrays.asList("N", "S", "E", "W").contains(spl[1]) && spl.length > 2 && !Arrays.asList("RD", "ROAD", "ST", "STREET", "AVE", "AV", "AVENUE").contains(spl[2])) {
                h = h + spl[1];
        ...
        while (h.length() > 1 && h.substring(h.length() - 1, h.length()).equals("-")) {
        ...
        if (!Parser.anyalpha(spl[0]) && spl.length > 1 && this.houses.size() == 1) {
            ...
            if (spl[1].replaceAll("AVENUE|STREET|ROAD|DRIVE|PLACE|BOULEVARD|APARTMENT|BASEMENT|BUILDING|FLOOR|FRONT|SUITE|LANE", "").equals(spl[1])) {
                ...
                if (firstHouse.num >= Long.parseLong(spl[0]) || Long.parseLong(spl[0]) - firstHouse.num >= 10) {
