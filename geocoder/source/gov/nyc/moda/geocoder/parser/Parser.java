...
public class Parser {
    ...
    public static String cleanStreet(String s) {
        ...
        if (s.indexOf("AVE E") < 0 && s.indexOf("AVENUE E") < 0) {
            s = s.replaceAll("(\\b|^)E(\\b|$)", "EAST");
        }
        if (s.indexOf("AVE W") < 0 && s.indexOf("AVENUE W") < 0) {
            s = s.replaceAll("(\\b|^)W(\\b|$)", "WEST");
        }
        if (s.indexOf("AVE N") < 0 && s.indexOf("AVENUE N") < 0) {
            s = s.replaceAll("(\\b|^)N(\\b|$)", "NORTH");
        }
        if (s.indexOf("AVE S") < 0 && s.indexOf("AVENUE S") < 0) {
            s = s.replaceAll("(\\b|^)S(\\b|$)", "SOUTH");
        }
        ...
        if (s.replaceAll("(\\b|^)ST", "").equals(s) && s.indexOf("BEACH") >= 0) {
            ...
            matcher = pattern.matcher(s);
            if (matcher.find()) {
                s = s.substring(0, matcher.end()) + "STREET" + s.substring(matcher.end() + 1);
            }
        }
        ...
        for (int i = 0; i < numberwords.length; i++)
            s = s.replaceAll("(\\b|^)" + numberwords[i], Integer.toString(i + 1) + " ");
        s = compbl(s);
